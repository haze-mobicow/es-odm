...
function Model(connection, defaults, opt, schema) {
    ...
    connection.indices.exists({index: this.index}, function (err, indexTest) {
        ...
        if (!indexTest) {
            console.log('creating index %s',this.index);
            connection.indices.create({index: this.index},function(err,idx){
                console.log('created index %s Error: %s',this.index,err);
                connection.indices.putMapping({index: this.index, type: this.type, body: mapping})
            });
        }else {
            connection.indices.getSettings({index: this.index, name: "index.mapper.dynamic"}, function (err, settings) {
